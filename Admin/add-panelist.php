  ...
  if ($username === "" || $fullName === "" || $password === "") {
    ...
    if ($errorMessage === "") {
      ...
      if ($passwordHash === false) {
        ...
        if ($insertStmt) {
          $insertStmt->bind_param("ssss", $username, $fullName, $passwordHash, $status);
          if ($insertStmt->execute()) {
            $successMessage = "Panelist account created.";
          } else {
            $errorMessage = "Unable to save the account.";
          }
          $insertStmt->close();
    ...
    <div class="min-h-screen bg-gradient-to-br from-white via-blue-50 to-slate-100">
        ...
        <div class="mx-auto flex max-w-3xl items-center justify-between px-4 py-3">
          <div class="flex items-center gap-3">
            ...
            <div class="text-xs">
              ...
              <p class="text-[11px] text-blue-100">Add Panelist Account</p>
            </div>
          </div>
          <button
            type="button"
      ...
      <main class="mx-auto max-w-3xl px-4 py-10">
        <div class="rounded-2xl border border-[#0d8ddb] bg-white p-6 shadow-sm">
          <h1 class="text-lg font-semibold text-[#052c6a]">Create Panelist Account</h1>
          <p class="text-xs text-[#052c6a]">
            Provide account details to add a new panelist.
          </p>
