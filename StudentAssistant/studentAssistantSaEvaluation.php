...
foreach ($evaluationSections as $sectionKey => $section) {
  $ratingGroups[$sectionKey] = array_keys($section["criteria"]);
  $ratingFieldNames = array_merge($ratingFieldNames, $ratingGroups[$sectionKey]);
}
$formRatings = [];
...
function saEnsureDepartmentHeadEvaluationTable(mysqli $conn): bool
{
  $createTableSql = "CREATE TABLE IF NOT EXISTS department_head_evaluations (
    ...
    evaluator_role VARCHAR(50) NOT NULL DEFAULT 'department_head',
    ...
    UNIQUE KEY uniq_application_head_role_term (application_id, head_username, evaluator_role, semester, school_year)
  ...
  if (function_exists("isgEnsureDepartmentHeadEvaluationsRoleColumn") && !isgEnsureDepartmentHeadEvaluationsRoleColumn($conn)) {
    return false;
  }
...
if ($headUsername !== "") {
  $headAccountStmt = $conn->prepare("SELECT name, lastname, full_name, office FROM users WHERE username = ? AND role = ? AND status = 'active' LIMIT 1");
  if ($headAccountStmt) {
    $headAccountStmt->bind_param("ss", $headUsername, $headRole);
...
function saLoadActiveScholarRecord(mysqli $conn, int $recordId, array $evaluatorScope): ?array
{
...
if ($applicationId === 0) {
  ...
  if (!$hasScholarTable) {
    ...
    if (is_array($resolvedScholarRow)) {
      ...
      $loadError = "Student assistant record not available for this evaluator.";
    }
  }
}
...
if ($loadError === "" && $hasEvaluationTable) {
  $existingStmt = $conn->prepare(
    "SELECT ratings_json, strengths, areas_improvement, recommendations, signature_data, evaluation_date
     ...
     WHERE application_id = ? AND head_username = ? AND evaluator_role = ? AND semester = ? AND school_year = ?
     LIMIT 1"
  );
  if ($existingStmt) {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($loadError !== "") {
    ...
    $sectionTotals = [];
    foreach ($ratingGroups as $sectionKey => $fields) {
      $sectionTotals[$sectionKey] = 0;
      foreach ($fields as $fieldName) {
        $sectionTotals[$sectionKey] += (int)($formRatings[$fieldName] ?? 0);
      }
    }
    $sectionATotal = (int)($sectionTotals["a"] ?? 0);
    $sectionBTotal = (int)($sectionTotals["b"] ?? 0);
    $sectionCTotal = (int)($sectionTotals["c"] ?? 0);
    ...
    $saveStmt = $conn->prepare(
      "INSERT INTO department_head_evaluations (
        application_id, applicant_name, semester, school_year, assigned_office, head_username, evaluator_role, head_name,
...
<body class="bg-white text-[#0b1b3a] font-sans">
    <div class="min-h-screen page-shell">
      ...
      >
        ...
        <nav class="flex-1 mt-2">
          <ul class="text-xs font-semibold">
            ...
            <li class="panel-nav-item active gap-2 cursor-pointer" onclick="window.location.href='studentAssistant-my-sas.php'">
              <i class="fas fa-user-friends w-5"></i>
              <span>My SA's</span>
            </li>
            <li class="panel-nav-item gap-2 cursor-pointer" onclick="window.location.href='studentAssistant-show-evaluation.php'">
      ...
      <main class="ml-0 md:ml-64 flex flex-col min-h-screen bg-[#eef2f7] pb-8 pt-14">
        ...
        <header class="border-b border-slate-200 px-10 py-8">
            ...
            <div class="mt-6 flex flex-wrap items-center gap-5 text-sm text-slate-700">
                <span class="font-semibold">Evaluator:</span>
                <?php foreach ($evaluatorOptions as $option): ?>
                  <span class="inline-flex items-center gap-2">
                    <span class="inline-flex h-4 w-4 items-center justify-center border border-slate-700 text-[10px] leading-none">
        ...
        <section class="px-10 py-6">
                ...
                <table class="min-w-full border-collapse">
                    ...
                    <tbody>
                        <?php foreach ($ratingOptions as $scale => $option): ?>
                          <tr class="<?= ((int)$scale % 2 === 0) ? "bg-slate-50" : "" ?>">
                              <td class="border border-slate-300 px-3 py-3 font-semibold"><?= htmlspecialchars((string)$scale) ?></td>
                              <td class="border border-slate-300 px-3 py-3"><?= htmlspecialchars($option["label"] . " (" . $option["short"] . ")") ?></td>
                              <td class="border border-slate-300 px-3 py-3 text-xs leading-5"><?= htmlspecialchars($option["interpretation"]) ?></td>
                          </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
                ...
                <table class="min-w-full border-collapse text-sm text-slate-700">
                    <thead>
                        <tr class="bg-slate-100">
                            ...
                            <?php foreach ($ratingOptions as $scale => $option): ?>
                              <?php $ratingHeader = ((int)$scale === 1) ? "NI" : $option["label"]; ?>
                              <th class="w-24 border border-slate-300 px-3 py-2 text-center font-semibold">
                                <?= htmlspecialchars($ratingHeader) ?><br><span class="text-xs">(<?= htmlspecialchars((string)$scale) ?>)</span>
                              </th>
                            <?php endforeach; ?>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($evaluationSections as $sectionKey => $section): ?>
                          <tr class="bg-blue-50/60 font-semibold">
                              <td colspan="<?= 2 + count($ratingOptions) ?>" class="border border-slate-300 px-3 py-2"><?= htmlspecialchars($section["title"]) ?></td>
                          </tr>
                          <?php $criterionIndex = 0; ?>
                          <?php foreach ($section["criteria"] as $fieldName => $criterion): ?>
                            <?php
                              $criterionIndex++;
                              $criterionParts = explode(" ", $criterion, 2);
                              $criterionCode = $criterionParts[0] ?? "";
                              $criterionText = $criterionParts[1] ?? $criterion;
                            ?>
                            <tr class="<?= ($criterionIndex % 2 === 0) ? "bg-slate-50" : "" ?>">
                                <td class="border border-slate-300 px-3 py-2"><?= htmlspecialchars($criterionCode) ?></td>
                                <td class="border border-slate-300 px-3 py-2"><?= htmlspecialchars($criterionText) ?></td>
                                <?php foreach ($ratingOptions as $scale => $option): ?>
                                  <td class="border border-slate-300 text-center">
                                      <input type="radio" name="<?= htmlspecialchars($fieldName) ?>" value="<?= htmlspecialchars((string)$scale) ?>" class="h-4 w-4 text-slate-900 focus:ring-yellow-400" />
                                  </td>
                                <?php endforeach; ?>
                            </tr>
                          <?php endforeach; ?>
                          <tr class="bg-slate-100 font-semibold">
                              ...
                              <td id="section-<?= htmlspecialchars($sectionKey) ?>-total" colspan="<?= count($ratingOptions) ?>" class="border border-slate-300 px-3 py-2 text-center font-semibold text-slate-800">0</td>
                          </tr>
                        <?php endforeach; ?>

                        <tr class="bg-slate-100 font-semibold">
                            <td class="border border-slate-300 px-3 py-2" colspan="2">Over-all Total</td>
                            <td id="overall-total" colspan="<?= count($ratingOptions) ?>" class="border border-slate-300 px-3 py-2 text-center font-semibold text-slate-900">0</td>
                        </tr>
                    </tbody>
                </table>

                <div class="border-x border-b border-slate-300 px-4 py-4 text-sm text-slate-700">
                    <p class="font-semibold">Performance Summary</p>
                    <div class="mt-3 grid gap-3 md:grid-cols-3">
                        <p>Overall Total Score: <span id="overall-total-summary" class="font-semibold text-slate-900">0</span>/80</p>
                        <p>Average Rating: <span id="average-rating" class="font-semibold text-slate-900">0.00</span></p>
                        <p>Performance Level: <span id="performance-level" class="font-semibold text-slate-900">Incomplete</span></p>
                    </div>
                </div>
            <div class="mt-8 space-y-6 text-sm text-slate-700">
                ...
                <div class="pt-2 text-sm text-slate-700">
                    ...
                    <p class="mt-2 leading-6 indent-8">A Student Assistant must obtain an overall performance rating of at least &quot;Good&quot; and comply with all provisions of the Student Assistant Scholarship Agreement to remain eligible for retention in the Student Assistant Scholarship Program.</p>
                </div>
            </div>
    ...
    <script>
      ...
      document.addEventListener("DOMContentLoaded", () => {
        ...
        const sectionTotalEls = Object.fromEntries(
          Object.keys(ratingGroups).map((sectionKey) => [
            sectionKey,
            document.getElementById(`section-${sectionKey}-total`),
          ])
        );
        const overallTotalEl = document.getElementById("overall-total");
        const overallTotalSummaryEl = document.getElementById("overall-total-summary");
        ...
        const totalRatingFields = Object.values(ratingGroups).reduce((sum, fields) => sum + (fields || []).length, 0);

        const updateTotals = () => {
          const sectionTotals = {};
          Object.entries(ratingGroups).forEach(([sectionKey, fields]) => {
            sectionTotals[sectionKey] = (fields || []).reduce((sum, fieldName) => sum + getFieldScore(fieldName), 0);
          });
          const overall = Object.values(sectionTotals).reduce((sum, total) => sum + total, 0);
          const ratedCount = Object.values(ratingGroups).reduce((sum, fields) => sum + (fields || []).filter((fieldName) => getFieldScore(fieldName) > 0).length, 0);
          const average = ratedCount > 0 ? overall / ratedCount : 0;
          let performanceLevel = "Incomplete";
          if (ratedCount === totalRatingFields) {
            if (average >= 4) {
          ...
          Object.entries(sectionTotals).forEach(([sectionKey, total]) => {
            if (sectionTotalEls[sectionKey]) sectionTotalEls[sectionKey].textContent = String(total);
          });
          if (overallTotalEl) overallTotalEl.textContent = String(overall);
          if (overallTotalSummaryEl) overallTotalSummaryEl.textContent = String(overall);
          if (averageRatingEl) averageRatingEl.textContent = average.toFixed(2);
          if (performanceLevelEl) performanceLevelEl.textContent = performanceLevel;
        };
