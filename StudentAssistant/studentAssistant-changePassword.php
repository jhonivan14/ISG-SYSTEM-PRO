...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if ($headUsername === "") {
    ...
    $evaluatorRole = headExpectedRole();
    $user = isgLoadEvaluatorUser($conn, $headUsername, $evaluatorRole);
    $storedHash = $user ? (string)($user["password_hash"] ?? "") : "";

    if (!$user) {
      $passwordError = "Unable to update password.";
    } elseif (!isgVerifyEvaluatorPassword($currentPassword, $storedHash)) {
      $passwordError = "Current password is incorrect.";
    } else {
      $newHash = password_hash($newPassword, PASSWORD_DEFAULT);
      if ($newHash === false) {
        $passwordError = "Unable to update password.";
      } elseif (!isgUpdateEvaluatorPassword($conn, $headUsername, $evaluatorRole, $newHash)) {
        $passwordError = "Unable to update password.";
      } else {
        if ($evaluatorRole === "department_head") {
          $legacyStmt = $conn->prepare("UPDATE head_offices SET password_hash = ? WHERE username = ? LIMIT 1");
          if ($legacyStmt) {
            $legacyStmt->bind_param("ss", $newHash, $headUsername);
            $legacyStmt->execute();
            $legacyStmt->close();
          }
        }
      ...
      >
        ...
        <nav class="flex-1 mt-2">
          <ul class="text-xs font-semibold">
            ...
            <li class="panel-nav-item active gap-2 cursor-pointer" onclick="window.location.href='studentAssistant-changePassword.php'">
